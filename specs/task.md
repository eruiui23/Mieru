...
- [ ] **Task 5.6: Frontend View-Switching Layout (Sidebar Router):** Implement a navigation state controller in the Streamlit sidebar. Clicking a navigation button must dynamically swap the central main page layout from the active "OCR Workspace" view to a dedicated, full-screen "View History Log" view displaying an interactive `st.dataframe` connected to the fetch endpoint. Selecting a historical log row must render the archived image side-by-side with its past extracted and translated texts.
