...
- [x] **Task 5.4: Backend Database Persistence Layer (SQLite):** Set up a lightweight SQLite database connection using a single table (`ocr_history`) that tracks fields: `id`, `timestamp`, `filename`, `engine`, `extracted_text`, `translated_text`, and `image_path` (TEXT). Configure the backend to save incoming image files to a local server directory (`backend/static/uploads/`).
- [x] **Task 5.5: Backend History Fetch Endpoint:** Expose a new API endpoint `GET /api/history` that queries the SQLite database and returns the log sorted by the most recent timestamp. Ensure the existing OCR endpoints automatically save incoming images to the local uploads directory and write a new row to the database with the path in `image_path` upon every successful processing cycle.
- [x] **Task 5.6: Frontend View-Switching Layout (Sidebar Router):** Implement a navigation state controller in the Streamlit sidebar. Clicking a navigation button must dynamically swap the central main page layout from the active "OCR Workspace" view to a dedicated, full-screen "View History Log" view displaying an interactive `st.dataframe` connected to the fetch endpoint. Selecting a historical log row must render the archived image side-by-side with its past extracted and translated texts.
