...
- [ ] **Task 5.4a: Independent Full Image Upload Endpoint:** Build a `POST /api/upload` endpoint that receives the raw, uncropped file immediately upon user drop, saves it to `backend/static/uploads/` with a unique UUID string, and returns this filename identifier to the frontend client.
