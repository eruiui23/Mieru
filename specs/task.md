...
- [ ] **Task 5.3: Frontend Accordion Dropdown (Option B):** Implement an `st.expander` named "文法 & 振り仮名 | Japanese Analysis (Furigana & Readings)" sitting right below the Extracted/Translated text block, which renders the payload (word, furigana, romaji, and meaning) inside a clean, conditional `st.dataframe`.
