...
- [ ] **Task 5.4: Backend Database Persistence Layer (SQLite):** Set up a lightweight SQLite database connection using a single table (`ocr_history`) that tracks fields: `id`, `timestamp`, `filename`, `engine`, `extracted_text`, `translated_text`, and `image_path` (TEXT).
