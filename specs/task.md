...
- [ ] **Task 5.6: Frontend View-Switching Layout (Sidebar Router):** Implement a navigation state controller in the Streamlit sidebar. Clicking a navigation button must dynamically swap the central main page layout from the active "OCR Workspace" view to a dedicated, full-screen "View History Log" view displaying an interactive `st.dataframe` connected to the fetch endpoint. The Streamlit workspace must utilize `st.session_state` to instantly trigger the `POST /api/upload` routine the moment a file hits the `st.file_uploader` box. When clicking "Run OCR", the client will send the tiny cropped byte slice alongside the cached `backend_filename` reference string to complete the state linkage. Selecting a historical log row must render the archived image side-by-side with its past extracted and translated texts.
