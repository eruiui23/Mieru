...
- [x] **Task 5.1: Backend Text Dissection Pipeline:** Integrate a text parser (like pykakasi) and a dictionary engine (like jamdict) into the backend services layer to dissect raw text strings into structured dictionaries containing words, furigana, rōmaji, and English definitions.
...
- [x] **Task 5.3: Frontend Accordion Dropdown (Option B):** Implement an `st.expander` named "文法 & 振り仮名 | Japanese Analysis (Furigana & Readings)" sitting right below the Extracted/Translated text block, which renders the payload (word, furigana, romaji, and meaning) inside a clean, conditional `st.dataframe`.
- [x] **Task 5.4: Backend Database Persistence Layer (SQLite):** Set up a lightweight SQLite database connection using a single table (`ocr_history`) that tracks fields: `id`, `timestamp`, `filename`, `engine`, `extracted_text`, `translated_text`, and `image_path` (TEXT).
- [x] **Task 5.4a: Independent Full Image Upload Endpoint:** Build a `POST /api/upload` endpoint that receives the raw, uncropped file immediately upon user drop, saves it to `backend/static/uploads/` with a unique UUID string, and returns this filename identifier to the frontend client.
- [x] **Task 5.5: Backend History Fetch Endpoint:** Expose a new API endpoint `GET /api/history` that queries the SQLite database and returns the log sorted by the most recent timestamp. Adjust the existing OCR endpoints (`POST /api/ocr` and `POST /api/ocr/compare`) to receive a `full_image_ref` parameter string alongside the cropped file stream. When inserting a successful log entry into SQLite, the `image_path` field should capture the folder link pointing to this full-size original reference file.
- [x] **Task 5.6: Frontend View-Switching Layout (Sidebar Router):** Implement a navigation state controller in the Streamlit sidebar. Clicking a navigation button must dynamically swap the central main page layout from the active "OCR Workspace" view to a dedicated, full-screen "View History Log" view displaying an interactive `st.dataframe` connected to the fetch endpoint. The Streamlit workspace must utilize `st.session_state` to instantly trigger the `POST /api/upload` routine the moment a file hits the `st.file_uploader` box. When clicking "Run OCR", the client will send the tiny cropped byte slice alongside the cached `backend_filename` reference string to complete the state linkage. Selecting a historical log row must render the archived image side-by-side with its past extracted and translated texts.
...
- [x] **Task 6.4: Refactor Persistence Layer to use SQLAlchemy ORM**
